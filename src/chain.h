...
extern bool TestNet();
extern const int64_t multiAlgoDiffChangeTarget;
...
extern unsigned int GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock,int algo);
...
public:
	...
	uint256 GetBlockPoWHash(int algo) const
	{
		return GetBlockHeader().GetPoWHash(algo);
	}
